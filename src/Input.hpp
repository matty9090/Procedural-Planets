...
class Input {
	public:
		...
		enum EKeyState { kNotPressed, kPressed, kHeld };
		...
		static void KeyDownEvent(EKeyCode Key);
		static void KeyUpEvent(EKeyCode Key);

		static bool KeyHit(EKeyCode eKeyCode);
		static bool KeyHeld(EKeyCode eKeyCode);

		unsigned int mouseX = 0;
		unsigned int mouseY = 0;

	private:
		static EKeyState keyStates[kMaxKeyCodes];
};
